...
case class MRNAllocatedRootLevel(
  ...
  def toXml: NodeSeq = {
    <SynIdeMES1>{SynIdeMES1}</SynIdeMES1>
      ...
      {
        SenIdeCodQuaMES4.map(value => <SenIdeCodQuaMES4>{value}</SenIdeCodQuaMES4>).getOrElse(NodeSeq.Empty)
      } ++
      <MesRecMES6>{MesRecMES6}</MesRecMES6> ++
      {
        RecIdeCodQuaMES7.map(value => <RecIdeCodQuaMES7>{value}</RecIdeCodQuaMES7>).getOrElse(NodeSeq.Empty)
      } ++
      ...
      {
        RecRefMES12.map(value => <RecRefMES12>{value}</RecRefMES12>).getOrElse(NodeSeq.Empty)
      } ++ {
      ...
      } ++
      {
        ...
        AckReqMES16.map(value => <AckReqMES16>{value}</AckReqMES16>).getOrElse(NodeSeq.Empty)
      } ++
      {
        ComAgrIdMES17.map(value => <ComAgrIdMES17>{value}</ComAgrIdMES17>).getOrElse(NodeSeq.Empty)
      } ++
      {
      ...
      <MesTypMES20>GB014A</MesTypMES20> ++
      {
        ...
        MesSeqNumMES22.map(value => <MesSeqNumMES22>{value}</MesSeqNumMES22>).getOrElse(NodeSeq.Empty)
      } ++ {
        FirAndLasTraMES23.map(value => <FirAndLasTraMES23>{value}</FirAndLasTraMES23>).getOrElse(NodeSeq.Empty)
      }
  }
}
