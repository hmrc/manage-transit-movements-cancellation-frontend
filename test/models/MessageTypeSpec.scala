...
class MessageTypeSpec extends SpecBase with ScalaCheckPropertyChecks {
  ...
  "must deserialise" - {
    "when IE015" in {
      ...
      json.validate[MessageType].get mustBe DepartureNotification
    }

    "when IE028" in {
      val json = JsString("IE028")
      json.validate[MessageType].get mustBe AllocatedMRN
    }

    "when IE055" in {
      val json = JsString("IE055")
      json.validate[MessageType].get mustBe GuaranteeRejected
    }
    ...
    "when IE004" in {
      ...
      json.validate[MessageType].get mustBe AmendmentAcceptance
    }
    ...
    "when something else" in {
      forAll(Gen.alphaNumStr) {
        code =>
          ...
          json.validate[MessageType].get mustBe Other(code)
      }
    }
  }
}
